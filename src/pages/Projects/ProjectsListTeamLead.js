...
function ProjectsListTeamLead() {
  const [projects, setProjects] = useState([]);
  const { authTokens, logoutUser } = useContext(AuthContext);
  const [searchName, setSearchName] = useState("");
  useEffect(() => {
    const getProjectsListTeamLead = async () => {
      try {
        ...
        const response = await fetch(
          `${process.env.REACT_APP_URL}/api/project/team-lead/list/?${queryString}`,
          {
            method: "GET",
            headers: {
        ...
        if (response.ok) {
          ...
          console.error("Error fetching project task. Status:", response.status);
        }
      } catch (error) {
    ...
    getProjectsListTeamLead();
  }, [searchName, authTokens.access, logoutUser]);
  return (
...
export default ProjectsListTeamLead;
